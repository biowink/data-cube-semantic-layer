...
FROM der.events
WHERE derived_tstamp BETWEEN DATE '2025-09-01' AND DATE '2025-09-08'
...
SELECT LEAST(CEILING(number_of_predicted_days * 1.0/2), 9) AS number_of_predicted_days,
       ...
       COUNT(DISTINCT events.analytics_id) * 1.0/COUNT(DISTINCT prediction_checkers.analytics_id) AS revisit_sessions
...
LEFT JOIN der.events ON prediction_checkers.analytics_id = events.analytics_id AND events.derived_tstamp >= DATE '2025-09-01'
    ...
    AND mobile_event_name = 'Show Cramps Prediction Screen'
INNER JOIN der.sessions ON prediction_checkers.analytics_id = sessions.analytics_id AND sessions.session_start >= DATE '2025-09-01'
    AND DATE_DIFF('day', DATE(prediction_checkers.derived_tstamp), DATE(sessions.session_start)) BETWEEN 1 AND 14 
WHERE rnk = 1
GROUP BY 1
ORDER BY 1
;