WITH experiment_ids AS (
    ...
    WHERE
        ...
        AND derived_tstamp >= '2023-04-01'
        AND experiment_id IN ('aatest_1','aatest_2','aatest_3',
                              'aatest_4','aatest_5','aatest_6',
                              'aatest_7','aatest_8','aatest_9','aatest_10')
    ...
    SELECT
           ...
           MIN(CASE WHEN experiment_id = 'aatest_6' AND variation_id IN ('0', '1') THEN variation_id ELSE '9' END) || '-' ||
           MIN(CASE WHEN experiment_id = 'aatest_7' AND variation_id IN ('0', '1') THEN variation_id ELSE '9' END) || '-' ||
           MIN(CASE WHEN experiment_id = 'aatest_8' AND variation_id IN ('0', '1') THEN variation_id ELSE '9' END) || '-' ||
           MIN(CASE WHEN experiment_id = 'aatest_9' AND variation_id IN ('0', '1') THEN variation_id ELSE '9' END) || '-' ||
           MIN(CASE WHEN experiment_id = 'aatest_10' AND variation_id IN ('0', '1') THEN variation_id ELSE '9' END)
