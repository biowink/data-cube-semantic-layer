...
SELECT experiment_id,
       ...
       COUNT(DISTINCT CASE WHEN mobile_event_name = 'Subscription Started' THEN events.analytics_id END) AS count_activity,
...
LEFT JOIN der.events ON users.analytics_id = events.analytics_id
                                      AND events.derived_tstamp > entered_experiment_ts
                                        ...
                                        AND mobile_event_name IN ('Subscription Started')
                                        AND DATEDIFF('day', entered_experiment_ts, derived_tstamp) = index
WHERE index < DATEDIFF('day', entered_experiment_ts, '2024-05-22') AND experiment_id = '2404_daily_checkin_android'
GROUP BY 1, 2, 3
ORDER BY 1, 2, 3
;