WITH activity_level AS (
    ...
    WHERE
        ...
        AND DATEDIFF('day', expires_at::DATE, lifetime.date) IN (-30, 60)
...
SELECT is_rh_mode,
       ...
       AVG(CASE WHEN days_to_expiry = 60 THEN share_wau END)/
        AVG(CASE WHEN days_to_expiry = -30 THEN share_wau END) AS wau_uplift,
       AVG(CASE WHEN days_to_expiry = 60 THEN share_mau END)/
        AVG(CASE WHEN days_to_expiry = -30 THEN share_mau END) AS mau_uplift
FROM activity_level
GROUP BY 1, 2
ORDER BY 1, 2
;