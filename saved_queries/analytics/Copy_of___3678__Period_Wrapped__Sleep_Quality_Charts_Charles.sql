SELECT
    CASE WHEN latitude > 0 THEN 'Northern Hemisphere' ELSE 'Southern Hemisphere' END AS hemisphere,
    MONTH(backend_updated_at) AS month,
    AVG(CASE WHEN type IN ('woke_up_tired') THEN 1.0 ELSE 0 END) AS share_woke_up_tired,
    AVG(CASE WHEN type IN ('trouble_falling_asleep') THEN 1.0 ELSE 0 END) AS share_trouble_falling_asleep
