WITH yearly_subscribers AS (SELECT master_id,
                            ...
                            WHERE subscription_type = 'Subscription Purchased'
                              ...
                              AND DATEDIFF('day', first_seen::DATE, backend_created_at::DATE) <= 30),
...
SELECT CASE WHEN renewed = 1 THEN 'Renewer' ELSE 'Churner' END     AS subscriber_status,
       DATEDIFF('month', started_dt, start::DATE) + 1              AS months_into_subscription,
       COUNT(DISTINCT sp_sessions.master_id)::FLOAT / total_cohort AS mau
FROM renewals
         LEFT JOIN der.sp_sessions ON renewals.master_id = sp_sessions.master_id AND
                                      sp_sessions.start::DATE BETWEEN started_dt AND expires_dt
         INNER JOIN cohort_sizes USING (renewed)
GROUP BY 1, 2, total_cohort
HAVING months_into_subscription BETWEEN 1 AND 12
ORDER BY 1, 2
;