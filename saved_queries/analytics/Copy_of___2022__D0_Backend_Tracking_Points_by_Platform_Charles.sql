WITH tracking_combined AS (
    ...
    SELECT
        ...
        sp_users.master_id,
        first_platform,
        COUNT(backend_tracking.master_id) AS count_tracking_points
    FROM der.sp_users
    LEFT JOIN der.backend_tracking
            ON sp_users.master_id = backend_tracking.master_id AND DATEDIFF('day', first_seen, backend_tracking.backend_updated_at) = 0 
