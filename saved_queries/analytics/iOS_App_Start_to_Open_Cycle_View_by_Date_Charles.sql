WITH session_starts AS (
    ...
    FROM der.events
    WHERE
        derived_tstamp >= CURRENT_DATE - INTERVAL '60' DAY
        AND derived_tstamp >= DATE('2024-04-06')
...
SELECT DATE(app_start_ts) AS date,
...
WHERE DATE(app_start_ts) > DATE_ADD('day', 1, DATE(account_created_at)) AND platform = 'ios'
GROUP BY 1
HAVING COUNT(*) > 1000
ORDER BY 1
;