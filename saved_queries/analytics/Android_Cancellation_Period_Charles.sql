...
WHERE subscription_history.subscription_source = 'mobile'
    ...
    -- AND subscription_history.country = 'United States'
...
SELECT
    ...
    AVG(CASE WHEN DATE_DIFF('day', first_purchased_at, first_canceled_ts) > 300 AND first_canceled_ts < DATE_ADD('year', 1, started_at) 
        AND cumulative_subscription_duration = 12 THEN 1.0 ELSE 0 END) AS cancelation_diff3
FROM subscriptions
WHERE first_purchased_at < DATE '2025-04-01' 
