...
did_create_account_method = 'email' AS email_signup,
    COUNT(DISTINCT CASE WHEN DATE_DIFF('day', account_created_at, show_tracking_block_modal_ts) = 0 THEN users.analytics_id END) * 1.0/COUNT(DISTINCT users.analytics_id) AS d1_retention_rate
FROM der.users
