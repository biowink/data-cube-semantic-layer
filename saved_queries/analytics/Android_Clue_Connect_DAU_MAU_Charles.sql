...
sharers AS (
    ...
    FROM
        der.events
    WHERE
        mobile_event_name = 'Did Share Clue Connect Code'
        AND derived_tstamp >= DATE '2023-09-01'
        AND major_app_version > 100
    GROUP BY
        1,
...
WHERE calendar.date < CURRENT_DATE AND day_is_first_of_month
    AND calendar.date >= DATE_TRUNC('month', first_connect_ts)
    AND sharers.platform = 'android'
GROUP BY 1, 2, 3, 4
)
SELECT months_into_lifecyle, 'viewer' AS user_type, AVG(dau_mau) AS dau_mau
FROM partners_cohorted
GROUP BY 1
HAVING COUNT(*) > 1
UNION ALL
SELECT months_into_lifecyle, 'sharer' AS user_type, AVG(dau_mau) AS dau_mau
