...
filtered_purchase_events as (
    ...
    where last_subscription_type in ('Subscription Purchased', 'Subscription Renewed')
      and DATE_DIFF('day', last_expires_at, backend_created_at) >= 56 -- 8 weeks
...
expirations as (
    select
        date_trunc('month', dateadd('day', 56, expires_at)) as dt,
