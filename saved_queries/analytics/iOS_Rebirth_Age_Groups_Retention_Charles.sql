...
SELECT age_group,
       ...
       COUNT(DISTINCT sessions.analytics_id || CAST(DATE(session_start) AS varchar)) * 1.0/COUNT(DISTINCT sessions.analytics_id) AS dau_mau
FROM age_grouped_users
CROSS JOIN (SELECT (ROW_NUMBER() OVER (ORDER BY date) - 1) * 6 AS months_into_lifecycle FROM static.calendar LIMIT 4) AS month_indices
