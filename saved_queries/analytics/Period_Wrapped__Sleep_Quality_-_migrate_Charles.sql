WITH total_sleep_days AS (
    SELECT COUNT(*) AS count_sleep_days
...
SELECT
    type,
    COUNT(*) AS count_sleep_days,
    COUNT(*) * 1.0/(SELECT count_sleep_days FROM total_sleep_days) AS share_sleep_days
FROM der.backend_tracking
INNER JOIN user_metrics.user_last_optional_consent_status USING (analytics_id)
WHERE
    backend_updated_at BETWEEN DATE '2024-10-30' AND DATE '2025-10-30'
