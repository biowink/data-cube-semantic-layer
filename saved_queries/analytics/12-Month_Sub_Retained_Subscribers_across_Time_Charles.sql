...
SELECT DATE_TRUNC('quarter', subscriber_cohort) AS subscriber_cohort,
       DATE_DIFF('month', subscriber_cohort, month) AS subscription_month,
       SUM(is_retained) AS cohort_retained
FROM retained_cross
WHERE subscription_month IN (1, 13, 25) AND month < DATE_TRUNC('month', CURRENT_DATE)
GROUP BY 1, 2
ORDER BY 1, 2
;