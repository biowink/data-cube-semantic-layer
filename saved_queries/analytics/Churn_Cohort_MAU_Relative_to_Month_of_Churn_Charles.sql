...
SELECT DATE_TRUNC('month', expired_dt) AS churn_month_cohort,
       month_offset,
       COUNT(DISTINCT master_id) AS churning_users,
       NULLIF(COUNT(DISTINCT CASE WHEN DATEDIFF('month', expired_dt, session_dt) = month_offset THEN master_id END)::FLOAT/
         COUNT(DISTINCT CASE WHEN month_offset >= 0 THEN master_id
             WHEN month_offset < 0 AND DATEDIFF('month', expired_dt, first_seen_dt) <= month_offset THEN master_id END), 0) AS mau
FROM subscriber_sessions
CROSS JOIN offset_array
WHERE DATE_TRUNC('month', expired_dt) >= '2021-10-01'
GROUP BY 1, 2
ORDER BY 1 DESC, 2
;