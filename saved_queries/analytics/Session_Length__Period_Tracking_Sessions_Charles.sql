WITH period_trackers AS (
    SELECT
        ...
        MIN(CASE
                WHEN mobile_event_name = 'Open Option Modal' AND
    ...
    GROUP BY 1
    HAVING exit_option_modal_ts IS NOT NULL AND exit_data_entry_ts IS NOT NULL
),
    pdt AS (
        SELECT
            session_length_sec,
...
SELECT session_length_sec,
       SUM(freq) OVER (ORDER BY session_length_sec ASC ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS cum_dens
FROM pdt
ORDER BY 1
LIMIT 180
;